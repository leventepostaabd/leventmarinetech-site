...
function Badge({ tone, scale, className }: Required<Omit<LogoLockupProps, 'variant'>>) {
  const card =
    tone === 'light'
      ? 'bg-navy-900/55 ring-white/15 backdrop-blur'
      : 'bg-navy-700 ring-navy-700';
  const head = tone === 'light' ? 'text-white' : 'text-white';
  const kicker = tone === 'light' ? 'text-amber-300' : 'text-amber-300';

  return (
    <div
      className={`inline-flex items-center gap-3 rounded-xl px-3 py-2 ring-1 ${card} ${className}`}
      style={{ transform: `scale(${scale})` }}
    >
      <LeventLogo size={32} tone="light" />
      <div className="flex flex-col leading-none">
        <div className={`font-head font-black uppercase tracking-[0.08em] text-[15px] ${head}`}>
          LEVENT&nbsp;MARINE
        </div>
        <div className={`mt-1.5 font-mono text-[8.5px] uppercase tracking-[0.28em] ${kicker}`}>
          Electro-Tech · 24/7 Worldwide
        </div>
      </div>
    </div>
  );
