...
export default async function PortalRfqs() {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-5">
        <h2>My supply RFQs</h2>
        <Link href="/supply-wizard" className="btn-accent btn-sm">New RFQ</Link>
      </div>
      {error && <div className="card text-red-600 text-sm">Error: {error.message}</div>}
      {(data?.length ?? 0) === 0 ? (
        <div className="card text-ink-muted">No RFQs yet.</div>
      ) : (
        <div className="overflow-x-auto card !p-0">
          <table className="w-full text-[13px]">
            <thead className="bg-navy-50 text-left">
              <tr>{['Date', 'Kind', 'Urgency', 'Brand · Part', 'Vessel · Port', 'Status'].map((h) => (
