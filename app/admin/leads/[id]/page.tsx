      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <Link href={`/admin/leads?tab=${lead.track}`} className="font-mono text-[12px] text-ink-subtle hover:text-ink no-underline">
            ← {lead.track === 'service' ? 'Servis' : 'Tedarik'} adaylarına dön
          </Link>
          <h2 className="text-[22px] mt-2 leading-tight">
            {lead.company?.name ?? <span className="text-ink-subtle italic">Firma yok</span>}
            {lead.vessel && (
          ...
          <div className="flex flex-wrap gap-2 mt-2 text-[11px] font-mono uppercase tracking-[0.1em]">
            ...
            <span className="px-2 py-0.5 rounded-full bg-navy-50 text-ink-subtle">{lead.track === 'service' ? 'Servis' : 'Tedarik'}</span>
            <span className="px-2 py-0.5 rounded-full bg-amber/15 text-amber-700">puan {lead.priority_score}</span>
          </div>
        </div>
      </header>
